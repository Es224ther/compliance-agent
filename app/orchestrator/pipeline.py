...
from app.guards.field_rules import generate_followup_questions
from app.orchestrator import router
from app.processors import escalation_checker, report_generator
...
async def run_pipeline(
    ...
    try:
        ...
        if intake_result.requires_followup and intake_result.followup_prompt:
            state.followup_questions = _build_followup_questions(intake_result.missing_fields)
            await _emit_event(
                progress_callback,
                {
                    "step": "followup",
...
async def _continue_from_analysis(
    ...
    await _emit_event(
        progress_callback,
        {"step": "risk_analysis", "status": "completed", "message": "风险分析完成"},
    )
...
def _build_followup_questions(missing_fields: list[str]) -> list[FollowUpQuestion]:
