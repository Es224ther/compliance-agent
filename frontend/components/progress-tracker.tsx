...
function buildSteps(events: ProgressEvent[]) {
  ...
  return STEP_DEFS.map((d) => {
    ...
    return { ...d, status: (ev?.status ?? "pending") as StepStatus, message: ev?.message };
  });
}
...
function StatusDot({ status }: { status: StepStatus }) {
  ...
  if (status === "running") {
    return (
      <span className="shrink-0 w-5 h-5 rounded-full flex items-center justify-center" style={{ background: "rgba(99,102,241,0.12)", border: "1px solid rgba(99,102,241,0.3)" }}>
        ...
        {steps.map((s) => (
          <li key={s.key} className="flex items-center gap-3" style={{ opacity: s.status === "pending" ? 0.35 : 1, transition: "opacity 250ms ease" }}>
            ...
            <div className="flex-1 min-w-0 flex items-center gap-2">
              ...
              {s.status === "running" && (
                <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: "rgba(99,102,241,0.12)", color: "#818cf8", border: "1px solid rgba(99,102,241,0.2)" }}>
                  进行中
                </span>
              )}
            </div>
